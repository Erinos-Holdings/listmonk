...
func installCampaign(campTplID, archiveTplID int, q *models.Queries) {
	...
	if _, err := q.CreateCampaign.Exec(uuid.Must(uuid.NewV4()),
		...
		nil,
		json.RawMessage("[]"),
		// Fork (LIST-GRID-SPEC D11) -- attribs. This insert bypasses core.CreateCampaign and its
		// language default, and a language-less regular campaign cannot start.
		json.RawMessage(`{"lang": "`+models.CampaignLangDefault+`"}`),
