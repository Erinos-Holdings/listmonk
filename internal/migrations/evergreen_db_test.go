...
func newEvergreenHarness(t *testing.T) *evergreenHarness {
	...
	for i := 0; i < 2; i++ {
		...
		// Fork (click tracking) -- and v6.2.6.
		if err := V6_2_6(db, nil, nil, lo); err != nil {
			t.Fatalf("V6_2_6 run %d: %v", i+1, err)
		}
		// Fork (send retry) -- and v6.2.7.
		if err := V6_2_7(db, nil, nil, lo); err != nil {
			t.Fatalf("V6_2_7 run %d: %v", i+1, err)
		}
		// Fork (editor polish, templates.brand) -- and v6.2.8.
		if err := V6_2_8(db, nil, nil, lo); err != nil {
			t.Fatalf("V6_2_8 run %d: %v", i+1, err)
		}
		// Fork (media tags, media.tags) -- and v6.2.9.
		if err := V6_2_9(db, nil, nil, lo); err != nil {
			t.Fatalf("V6_2_9 run %d: %v", i+1, err)
		}
		// Fork (holds) -- and v6.2.10.
		...
		// Fork (system health) -- and v6.2.13 (a new table only).
		if err := V6_2_13(db, nil, nil, lo); err != nil {
