...
// ErrMaybeDelivered wraps a NON-reply error (write failure, EOF, timeout -- never an
// SMTP reply code) raised AFTER the message body was written and its terminator sent:
// the server may already have accepted the message before the error surfaced, so a
// retry could deliver it twice (erinos fork, SEND-RETRY-SPEC D10/F1). A 4yz/5yz reply
// at the same point is an explicit verdict and is classified normally.
// canRetry always refuses it; callers that track per-recipient state must treat it as
// "attempted, unconfirmed", never as "not sent".
var ErrMaybeDelivered = errors.New("error after message data was sent; the server may have accepted it")

// ErrPoolWaitTimeout is returned when no connection became free within
// PoolWaitTimeout. It is transient (every connection was busy) and retriable.
var ErrPoolWaitTimeout = errors.New("timed out waiting for free conn in pool")
...
func (c *conn) send(e Email) (bool, error) {
	...
	// Combine e-mail addresses from multiple lists. A malformed address is
	// permanent: no retry can make it parse (erinos fork, SEND-RETRY-SPEC D11).
	emails, err := combineEmails(e.To, e.Cc, e.Bcc)
	if err != nil {
		return false, err
	}

	// Extract SMTP envelope sender from the email struct. Permanent, as above.
	...
	// Close exactly once: textproto's DotWriter re-sends the terminator on a second
	// Close, and the stray reply that earns would be read as the next command's response
	// (RSET fails, a healthy connection is thrown away). Latent upstream -- the deferred
	// Close only re-fired on paths where the connection was dead anyway.
	isClosed = true
	if err := w.Close(); err != nil {
		// erinos fork, SEND-RETRY-SPEC D10 (refined 2026-09-10): the reply to the
		// end-of-data is the server taking or refusing responsibility. A 4yz/5yz REPLY
		// here is explicit -- RFC 5321 §4.2.1, 4yz = "the requested action did not
		// occur" -- so it is classified like any other reply; SES's "454 Throttling
		// failure" arrives exactly here. Only a NON-reply error (write failure, EOF,
		// timeout) is ambiguous: the server may have accepted the message before the
		// error surfaced. That one is wrapped as ErrMaybeDelivered and never retried.
		var tpErr *textproto.Error
		if errors.As(err, &tpErr) {
			return canRetry(err), err
		}
		return false, fmt.Errorf("%w: %w", ErrMaybeDelivered, err)
	}
...
// canRetry returns true if the given error is transient and the message can be
// retried on another attempt: network errors (TCP/DNS/timeout/broken pipe), a pool
// wait timeout, and — erinos fork, SEND-RETRY-SPEC D1 — an SMTP 4yz reply, which
// RFC 5321 §4.2.1 defines as "the command was not accepted, and the requested
// action did not occur", i.e. the sender is expected to try again. A 5yz reply is
// permanent. ErrPoolClosed is never retried (D11): backing off against a closing
// pool only stalls shutdown.
//
// After the DATA terminator conn.send applies this only to an SMTP REPLY; a non-reply
// error there is wrapped in ErrMaybeDelivered (refused above) because the server may
// have accepted the message before the error surfaced (D10). Keep it that way.
func canRetry(err error) bool {
	if err == nil {
		return false
	}
	if errors.Is(err, ErrPoolClosed) || errors.Is(err, ErrMaybeDelivered) {
		return false
	}
